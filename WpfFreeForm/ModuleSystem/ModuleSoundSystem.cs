...
namespace ModuleSystem {
    ...
    public class ModuleSoundStream : Stream {
        ...
        public ModuleSoundStream(uint sampleRate, bool stereo = false) {
            position = 0;
        ...
        /// </summary>
        private void WriteWavHeader(uint sampleRate, bool stereo = false) {
            ...
            uint waveSize = 4;
            uint subchunk2Size = int.MaxValue - 44; //)(int.MaxValue * blockAlign);
    ...
    public class PlayBackWaveProvider : IWaveProvider, IDisposable {
        private ConcurrentQueue<byte> sampleQueue;
        private AutoResetEvent dataAvailableSignaler = new AutoResetEvent(false);
        private int preloadSize = 2000;
        private WaveFormat waveFormat;

        public PlayBackWaveProvider(uint sampleRate, bool stereo = false) {
            sampleQueue = new ConcurrentQueue<byte>();
            waveFormat = new WaveFormat((int)sampleRate, 16, (stereo ? 2 : 1));
        }
